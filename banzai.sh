...
if [[ "${SECONDARY_INDEX}" == "YES" ]]; then
	...
	# Note sequences generated with Nextera libraries effectively have only a single index (by default), consisting of unique combination of the P5 and P7 index sequences
...
for (( i=0; i < "${#FILE1[@]}"; i++ )); do
  ...
  CURRENT_ID1_NAME=$( awk -F, '
	...
	################################################################################
	# CORRECT SEQUENCE ORIENTATION (cutadapt)
	################################################################################
		...
		echo $(date +%Y-%m-%d\ %H:%M) "Counting sequences in primer-finding input..."
		seq_N_demult_concat=$( grep -e '^>' --count "${FILTERED_OUTPUT}" )
		echo $(date +%Y-%m-%d\ %H:%M) "${seq_N_demult_concat}" "sequences found in primer-finding input"
		echo

		echo $(date +%Y-%m-%d\ %H:%M) "Correcting sequence orientation (so that all of the reads are in the same direction)..."

		(cutadapt\
			-a primerR1_found="${PRIMER1RC}" \
			-a primerR2_found="${PRIMER2RC}" --no-trim	-o "${FILTERED_OUTPUT}"-{name}.fasta "${FILTERED_OUTPUT}")
			#	\
		...
		seqtk seq -r "${FILTERED_OUTPUT}"-primerR1_found.fasta > "${FILTERED_OUTPUT}"-R1_RC.fasta
		rm "${FILTERED_OUTPUT}"-primerR1_found.fasta
		cat "${FILTERED_OUTPUT}"-R1_RC.fasta >> "${FILTERED_OUTPUT}"-primerR2_found.fasta
		rm "${FILTERED_OUTPUT}"-R1_RC.fasta
		mv "${FILTERED_OUTPUT}"-primerR2_found.fasta "${FILTERED_OUTPUT}"
	################################################################################
